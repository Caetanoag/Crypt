...
let binType = 8; // quantidade de bits que cada charCode precisa ter.
/**
 ...
 */
function gerarMAC(texto, senha) 
{
    let hashSenha = gerarHash(senha); 
    let seeds = getSeedsByHex(hashSenha.substring(0, 32));
 ...
 * @returns String
 */
function gerarInitializationVector()
{
    let iv = "";
    for(let i = 0; i < 16; i++)
    {
 ...
 */
function rotacionarDireita(num = 0, deslocar = 0) 
{
  return (((num >>> (deslocar % binType)) | (num << (binType - (deslocar % binType)))) &(2 ** binType - 1));
}
/**
 ...
 * @returns Inteiro rotacionado
 */
function rotacionarEsquerda(num = 0, deslocar = 0) 
{
  return (((num << (deslocar % binType)) | (num >>> (binType - (deslocar % binType)))) &(2 ** binType - 1));
}
/**
 ...
 */
function criptR(string, generator) 
{
  let valueList = [];
  ...
  for (let i = 0; i < string.length; i++) 
  {
    ...
    let rotationValue = (generator.next()%binType)+1;
    let tmp = rotacionarDireita(string.charCodeAt(i) ^ previousBlock, rotationValue);
 ...
 * @returns [] - charCodes
 */
function uncriptR(arr, generator) 
{
  let valueList = [];
  let previousBlock = generator.next() & 0xFF;
  for (let i = 0; i < arr.length; i++) 
  {
    ...
    let cifrado = arr[i];
    tmp ^= (randomXorKey >>> 0) & 0xFF
    tmp = rotacionarEsquerda(tmp, rotationValue);
    tmp ^= previousBlock;
...
function generate2DBinaryCode(word) 
{
 ...
 */
function insertTrash(word, h, generator) 
{ 
  let bin = generate2DBinaryCode(word);
  let out = [];
...
function criptTrashToString(arr) 
{
  while (arr.length % binType !== 0) 
  {
    arr.push(0);
  }
  let str = "";
  for (let i = 0; i < arr.length; i += binType) 
  {
    let add = parseInt(arr.slice(i, i + binType).join(""), 2).toString(16); 
    if(add.length < 2)
    {
      add = "0" + add;
    }
...
function criptT(word, trashing, generator) 
{
  let arr = flatten(insertTrash(word, trashing, generator));
  return arr;
}
/**
 ...
 */
function unflattenArrayWithTrash(arr, h) 
{
  let bits = [];
  for (let i = 0;i < arr.length;i += binType + trashSize*Math.floor((binType - 1) / h)) 
  {
    bits.push(arr.slice(i, i + binType + trashSize*Math.floor((binType - 1) / h)));
  }
  return bits;
}
/**
 ...
 */
function uncriptT(arr, h) 
{
  return flatten(removeTrash(unflattenArrayWithTrash(arr, h), h));
}
/**
 ...
 */
function criptAll(input) 
{
  let ins = input
  return criptTrashToString(criptT((criptR(ins, xor128Rotation)).join(","), trashing, xor128Numbers));
}
/**
...
function unHex(texto)
{
  ...
  {
    bin.push(getBin(parseInt(a[j], 16), 8));
  }
  let bits = [];
 ...
 * @returns 
 */
function arrNumberToString(arr)
{
  let out = "";
  for(let i = 0; i < arr.length; i++)
  {
 ...
 */
function uncriptAll(texto) 
{
  if(texto == "")
  {
    return ""
  }
  return arrNumberToString(uncriptR(bitsToCharCode(uncriptT(flatten(unHex(texto)),trashing)).split(","),xor128Rotation));
}
/**
 ...
 * @param {String} iv 
 */
function startXorGenerators(iv)
{
    let hash = gerarHash(chaveEntrada + iv); 
    let seedsTrashing = getSeedsByHex(hash.substring(0,64));
    xor128Trashing = new xorshift128(seedsTrashing[0], seedsTrashing[1], seedsTrashing[2], seedsTrashing[3]);
    trashing = (xor128Trashing.next()%8) + 1;
    let seedsNumbers = getSeedsByHex(hash.substring(192,256));
    xor128Numbers = new xorshift128(seedsNumbers[0], seedsNumbers[1],seedsNumbers[2], seedsNumbers[3]);
    let seedsRotation = getSeedsByHex(hash.substring(64,96));
    xor128Rotation = new xorshift128(seedsRotation[0], seedsRotation[1],seedsRotation[2], seedsRotation[3]);
    let seedsSize = getSeedsByHex(hash.substring(128,192));
    xor128Size = new xorshift128(seedsSize[0], seedsSize[1],seedsSize[2], seedsSize[3]);
    trashSize = (xor128Size.next()%8) + 1;
}
console.log("");
console.log("============ Criptografador Bitwise ============");
console.log("Aplicação de Matemática e segurança cibernética");
console.log("================================================")
console.log("");
...
{
    console.log("Opção inválida, insira 1 ou 2.")
    input = parseInt(prompt("--"));
}

console.log("Insira a chave: ");
let chaveEntrada = prompt("--");
console.log("Processando...")
let iv = gerarInitializationVector() + gerarInitializationVector() + gerarInitializationVector();
let xor128Trashing, trashing, trashSize;;
let xor128Numbers, xor128Rotation, xor128Size;
if(input === 1)
{
  startXorGenerators(iv);
  console.log("Insira o quer quer criptografar: ");
  let message = prompt("--");
  let crypt = criptAll(message)
  let mac = gerarMAC(crypt, chaveEntrada);
...
if(input === 2)
{
  console.log("Insira a mensagem criptografada: ");
  let arrMessage = prompt("--").split("-");
  let crypt = arrMessage[0];
  let macMessage = arrMessage[1];
  let iv = arrMessage[2];
  startXorGenerators(iv);
  let macEsperado = gerarMAC(crypt, chaveEntrada);
  if(!constantTimeCompare(macEsperado, macMessage))
  {
    console.log("Mensagem corrompida, adulterada ou chave incorreta. Impossível prosseguir.");
  }
  else
  {
    console.log("Mensagem descriptografada: ")
    console.log(String(uncriptAll(crypt)));
  }
}
